...
- R8 mapping snapshots: `~/.keynobi/mappings/<sha256>.txt`, kept while a record in the history names them; see [R8 Mapping Snapshots](#r8-mapping-snapshots).
...
- **Retention.** After the history is saved, `prune_snapshots` removes every snapshot `mappings_to_keep` does not return: today, the hashes the kept history (re-read from disk, so other processes' records count, and capped at `MAX_HISTORY`) names. Anything else that must outlive its record is added to that one function. It also runs on **Clear build history** and at startup (`rotate_persisted_build_logs`), removes temporary copies older than an hour (a process that died mid-copy), and, past `MAX_MAPPING_SNAPSHOTS` (32), removes the least recently saved (a dedupe hit counts as saved) with a warning.
