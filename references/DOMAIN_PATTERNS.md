...
- **Caps.** Per session: `MAX_EVENTS_PER_SESSION` (2,000), `MAX_BOOKMARKS_PER_SESSION` (100), and `MAX_SESSION_BYTES` (16 MiB of events); an event past a cap is counted in `droppedEvents` (a bookmark past it is refused). `get_debug_session` returns the newest `MAX_EVENTS_RETURNED` (500) with `eventsTruncated`; `list_debug_sessions` returns at most `MAX_SESSIONS`, newest first.
