...
Invariants that say "one at a time" or "the GUI sees it" hold **within one process**, so they cover the app and its attached sessions but not standalone servers. Across processes, only files in `~/.keynobi/` are shared; the per-project build lock in `build-locks/` and the per-device UI Automator lock in `ui-automator-locks/` are the cross-process guards.
...
- R8 mapping snapshots: `~/.keynobi/mappings/<sha256>.txt`, kept while a record in the history or an installed build names them; see [R8 Mapping Snapshots](#r8-mapping-snapshots).
- Installed builds: `~/.keynobi/installed-builds.json`, at most `MAX_INSTALLED_TARGETS` (16) device and package pairs; see [Installed Builds](#installed-builds).
- Debug sessions: `~/.keynobi/sessions/`, one per install on a device and package, at most `MAX_SESSIONS` (50), pruned by `sessions.retentionDays` (14) and `sessions.maxFolderMb` (200); see [Debug Sessions](#debug-sessions).
...
runAndDeploy()
  -> resolve_run_configuration (the active one; its plan heads the build log)
     (Ask or Last used with no online device: device picker, then resolve again with it)
  -> runBuild(plan.task)  (build:complete names the record: recordId)
  -> find_apk_path(variant, module, buildId = that recordId)
  -> install -> record_run_device
  -> launch per plan.launch  (launch_app_on_device with buildId = that recordId,
                              open_deep_link_on_device, or none)
  -> setLastLaunchedAt(package, plan.logcatFilter)
  -> finally: clear deployPhase

runBuildOnly()  (Build Only, Cmd+Shift+R)
  -> resolve_run_configuration(buildOnly) -> runBuild(plan.task)
```
...
- **Measured by Android.** `adb_manager::launch_app` starts a known component with `am start -W` and `parse_am_start_timing` reads `TotalTime`, `WaitTime`, and `LaunchState` (`COLD`, `WARM`, `HOT`, `RELAUNCH`; Android 10+). Missing fields are `None`, never 0: no `TotalTime` (the intent went to the activity already on top) or `Status: timeout` means no timing, and an absent or `UNKNOWN` launch state is `None`. The monkey and MAIN-intent fallbacks, and a run configuration's deep-link launch (`am start` without `-W`), report no timing.
...
- **Display times.** `LaunchTiming.displayedMs` (time to initial display, from `ActivityTaskManager`/`ActivityManager: Displayed <pkg>/<activity>: +…`) and `fullyDrawnMs` (time to full display, from `Fully drawn <pkg>/<activity>: +…`, which the app's `reportFullyDrawn` produces) come from this process's own logcat stream (`services/launch_display.rs`). Before `am start`, `LaunchWatch::start` records the next entry ID, and only when the stream reads the launched device (its serial, or a stream without a serial while that device is the only one online); otherwise both stay `None`. Only entries stored from then on, on that device (`device_of_entry`), with one of those two tags, and whose component package equals the launched package exactly count; the first of each wins. Durations are parsed as Android prints them (`+123ms`, `+1s234ms`, `+1m2s3ms`, largest unit first; anything else is ignored), preferring a `(total +…)` time. `restart_app`'s `display_time_ms` uses the same parser (`displayed_time_in_logcat`), on both tags. Both are `#[serde(default)]`, so older history loads with them `None`.
- **Not blocking the launch.** `launch_app_on_device` waits at most `DISPLAYED_GRACE` (1 s) after `am start -W` returned for the `Displayed` line (logged before `-W` returns, so this is only the stream's delay) and returns what arrived. When the result is recorded on a build and `Fully drawn` has not arrived, a detached task (`record_late_display_times`) checks the stream every 100 ms until `FULLY_DRAWN_WINDOW` (10 s) after the launch started, records what arrived through the same `attach_launch_timing` locked history update, and emits `build:launch_timing` (`LaunchTimingEvent`: `recordId`, `launch`); the frontend reloads the history. Nothing arrived means no update. Comparisons stay on `TotalTime`.
...
- **Debug sessions.** Every launch (Run App, `launch_app`, `restart_app`) is also added to the open debug session of its device and package, with its `LaunchTiming` when there is one, and display times that arrive later are added there too (`launchTiming`); see [Debug Sessions](#debug-sessions).

### R8 Mapping Snapshots

AGP writes `<module>/build/outputs/mapping/<variant>/mapping.txt` for a minified variant, and the next build of that variant overwrites it. `services/mapping_snapshots.rs` keeps a copy of each successful build's mappings, linked to its history record (`BuildRecord.mappings`, a list of `MappingSnapshot`: module, variant, `sha256`, `bytes`, `pg_map_id`), so a crash from that build's APK can be matched to the mapping that produced it.

- **What is taken.** Only a successful build's (`BuildFinalization.mappings` is ignored for a failed or cancelled one), both front doors alike. For each application module (`gradle_modules::application_modules`), `mapping/*/mapping.txt` under its `build/outputs`, modified at or after the build started (`MappingSource.build_started`, taken before Gradle spawns). An older file is the previous build's and is left alone.
- **Paths.** The module's `build/outputs` and its `mapping` directory are resolved with `validate_within_root` before they are listed, and each file with `resolve_project_file` inside the canonical `build/outputs`, so a symlinked module, `build`, `outputs`, `mapping`, variant directory, or file that leads outside is refused (a link inside is followed). The canonical file is opened with `O_NOFOLLOW`, and its size and mtime come from the open handle.
- **Storage.** `<data dir>/mappings/<sha256>.txt`, lowercase hex, so identical mappings (an unchanged app, two variants with the same code) are stored once. The copy is streamed in 1 MiB chunks through the hash into a `unique_tmp_path` file in that folder, never read into memory. `# pg_map_id:` is read from the leading `#` lines of the first 64 KiB; a missing or malformed value is `None`.
- **Locking.** The copy is written without the data lock, because a large mapping would hold up settings and history writes in every process. Publishing (renaming the copy to its hash name, or dropping it when that name exists), linking it to the record, saving the history, and pruning run in `persist_build_record_in`'s one critical section, so no process prunes a snapshot between its publish and its record. An unpublished `PreparedMapping` removes its copy when dropped.
- **Retention.** After the history is saved, `prune_snapshots` removes every snapshot `mappings_to_keep` does not return: the hashes the kept history (re-read from disk, so other processes' records count, and capped at `MAX_HISTORY`) names (`KeptMappings.referenced`), and the hashes the saved installs name and the debug sessions marked Keep name (`KeptMappings.pinned`, read from `installed-builds.json` and `sessions/index.json` in the same critical section; see [Installed Builds](#installed-builds) and [Debug Sessions](#debug-sessions)). Anything else that must outlive its record is added to that one function. It also runs on **Clear build history** (which keeps pinned snapshots) and at startup (`rotate_persisted_build_logs`), removes temporary copies older than an hour (a process that died mid-copy), and, past `MAX_MAPPING_SNAPSHOTS` (32) unpinned snapshots, removes the least recently saved unpinned ones (a dedupe hit counts as saved) with a warning. A pinned snapshot is never removed and does not count toward the backstop; installs pin at most `MAX_INSTALLED_TARGETS` (16) of them, one per install, and kept debug sessions at most `MAX_KEPT_SESSIONS` (5) more.
- **Caps.** `MAX_MAPPING_BYTES` (256 MiB): large apps' mappings are 50–150 MB, and the cap bounds one copy to about a second of I/O. A larger file is skipped with a logged reason. `MAX_MAPPINGS_PER_BUILD` (8) bounds a record's list.
- **Failures.** A snapshot that cannot be taken (unreadable, vanished, too large, disk full, permission denied) is logged with `tracing::warn!` and left off the record; the build is recorded and reported as usual. The copy runs in the blocking task that persists the history, off the async runtime; `build:complete` is emitted after it, so it waits for the copy itself and nothing else. The build status is updated before.
- **UI.** The past-build bar shows a badge per mapping: **R8 mapping saved: release (map id 6b1c2f0)**, the module named when there are several, and the details in its tooltip (`MappingSnapshotSummary`).

### Installed Builds

`services/installed_builds.rs` records which build produced the APK Keynobi installed on each device, so a crash on that device can later be matched to the build's R8 mapping, even after the build left the history.

- **APKs of a build.** In the same blocking finalization task as the mapping copies, before the data lock, `hash_build_apks` reads each application module's `build/outputs/apk/**/output-metadata.json` (at most 6 levels, `MAX_METADATA_DIRS` (64) folders; symlinked folders are not entered), resolves each `outputFile` with `resolve_project_file` inside the module's canonical `build/outputs`, opens it with `O_NOFOLLOW`, and streams a SHA-256. Only APKs modified at or after the build started count (`MappingSource.build_started`, the same freshness rule as mappings), and only for a successful build. The result is `BuildRecord.apks` (`BuiltApk`: module, variant, application ID, version code, `sha256`, bytes, path relative to the Gradle root), at most `MAX_APKS_PER_BUILD` (8), saved with the record under the lock. An APK that cannot be hashed is logged and left out; the build is recorded as usual.
- **Installs.** Both front doors install through `install_and_record`: Tauri `install_apk_on_device` (Run App) and MCP `install_apk` (attached or standalone). It hashes the canonical APK `validate_apk_within_build_outputs` returned while `adb install` runs. Only a successful install is recorded. Under the data lock, `record_install_in` re-reads the history and matches the newest record whose `apks` lists that hash, so any install of a recorded build's APK is matched, not only Run App's, and no IPC argument names the build. The entry (`InstalledBuild`: serial, AVD name, model, package, `apkSha256`, `buildId`, version code, `mappings`, `installedAt`) copies the record's `MappingSnapshot`s of the APK's module and variant whose snapshot file still exists. A recorded install then opens a debug session (see [Debug Sessions](#debug-sessions)).
- **Unmatched APKs.** An APK no kept record lists (built by Android Studio, or by a build that left the history) is recorded with `buildId: null` and no mappings. Its package and version code come from the `output-metadata.json` next to it, else aapt2. An install whose package cannot be read is logged and not recorded.
- **Device identity.** An emulator (`emulator-<port>`) is asked for its AVD name (`resolve_avd_name`) after the install rather than trusting a cached one, since serials are reused. Entries with AVD names match by AVD name, entries without by serial; one of each never match, so an emulator whose AVD did not answer is never taken for an AVD. A physical device keeps its serial.
- **Storage.** `installed-builds.json` in the data directory, one entry per (device, package): a later install replaces the earlier one. At most `MAX_INSTALLED_TARGETS` (16), the oldest dropped first. It is read and written under the data lock with `unique_tmp_path` and rename, like the history, so installs from several processes are all kept. It is not cleared with the build history. A missing or unreadable file reads as empty.
- **Retention.** The mappings the entries name are pinned (see [R8 Mapping Snapshots](#r8-mapping-snapshots)); an entry that is replaced or dropped releases its pin at the next prune.
- **Reading.** `installed_build(target, package)` returns the entry for a device and package; [Retrace](#retrace) compares its `apkSha256` with the device's APK and chooses its mapping from it, and also searches every entry for a map id or an APK hash. The frontend reads the whole list with `list_installed_builds`.
- **UI.** The past-build bar shows **Installed on Pixel_7 · 10:32** for each device where the viewed build's APK is still the last one Keynobi installed of its package (`InstalledBuildSummary`): the AVD name, else the model, else the serial; the time today, else the date. It requires the entry's `apkSha256` among the record's `apks`, so a build ID reused after the history was cleared is not mistaken for it. The tooltip has the package, serial, version code, and hash.
- **MCP.** `install_apk` adds a line naming what it recorded: `Recorded com.example on Pixel_7 as build #12; R8 mapping kept (6b1c2f0).`

### Retrace

`services/retrace.rs` deobfuscates a crash's stack trace with the Android SDK's R8 `retrace`, using the saved mapping of the build that produced the crashing app. The Tauri command `retrace_crash(crashGroupId)` (**Deobfuscate** in Entry Detail) and MCP `get_crash_stack_trace`/`get_crash_logs` with `retrace: true` all call `retrace_crash_group`, which returns a `RetraceOutcome`: `status` (`retraced`, `unavailable`, `refused`, `failed`), the trace (deobfuscated, else as logcat printed it), the build, the `MappingSnapshot` used, `matchedBy` (`mapId`, `deviceHash`, or `installRecord`), the device, the reason, and a one-line `summary` naming the build, module, variant, map id, and how the mapping was matched, for example `Deobfuscated with the R8 mapping of build #12 (:app release, map id 6b1c2f0…), matched by map id.` The summary shortens a long map id to its first 7 characters and an APK hash to its first 12, followed by `…`; MCP `map_id` is whole. Only a crash that left the logcat buffer is an error (`NotFound`); everything else is an outcome, so an agent's tool call never fails for it.

- **Never guess.** A wrong mapping gives plausible but wrong stacks, so every uncertainty refuses and returns the trace unchanged with the reason. The mapping is matched from the strongest source that applies, and a stronger source that names no saved mapping refuses rather than falling to a weaker one: (1) the map id in the trace; (2) the SHA-256 of the APK on the device; (3) only when the device cannot hash its APK, Keynobi's install record checked with `dumpsys`. The lookups (`services/retrace_match.rs`) scan the kept history and the saved installs; both are capped (`MAX_HISTORY` records of at most `MAX_MAPPINGS_PER_BUILD` mappings, `MAX_INSTALLED_TARGETS` installs), so there is no separate index to keep in step with retention. Only after a match is the cache consulted and the tool run.
- **Map id.** Since R8's June 2025 change ("Change the default emitted source file to 'r8-map-id-<MAP_ID>'"), R8 names every class it compiles `r8-map-id-<pg_map_id>` in its `SourceFile` attribute when no `-renamesourcefileattribute` or custom source file is set, so frames read `at a.a(r8-map-id-6b1c2f0…:12)`, and the default `pg_map_id` became the full 64-character SHA-256 of the mapping (`PG_MAP_ID_LENGTH` 64; it was 7). The id in the frame is the whole `pg_map_id`, not truncated. `trace_map_ids` reads the `<id>` of every `at …(r8-map-id-<id>[:line])` frame, and nothing else (exception messages are not read). No id goes on to the device hash. One id is looked up among the snapshots the kept history and the saved installs name (once per file, the newest record naming it giving the build): exactly one saved `pg_map_id` equal to it is used, with no device call; none is refused with **no saved mapping for map id …**, never another mapping; a snapshot file that is gone is refused. Only when no id is equal, an id of at least `MIN_MAP_ID_PREFIX_LEN` (7, the old id length) characters that starts exactly one longer saved id matches it (**matched by map id prefix …**); several, or a shorter id, is refused. Frames naming different ids are refused.
- **Device hash.** `adb shell pm path <package>` (validated and quoted), then, for exactly one `package:` path that is absolute, ends in `.apk`, is at most 4096 bytes and has no control characters, `adb shell sha256sum <path>` (quoted) with `ADB_APK_HASH_TIMEOUT` (60 s). Several paths are split APKs (an app bundle) and are refused. The hash is compared with the device's install record (`apkSha256`), then with the `apks` of the newest kept build record listing it (which recognises an APK Keynobi built and Android Studio or `adb install` installed; its mapping is the record's snapshot of that APK's module and variant), then with every other saved install (another device or package). A match whose build saved no mapping for that module and variant is refused; no match is refused ("not the one Keynobi installed … the app was reinstalled outside Keynobi", or "not written by a build Keynobi kept or installed"). The install record is not checked with `dumpsys` when the hash matches.
- **Fallback.** When the device cannot hash its APK (`sha256sum` missing on very old Android, a failed or unreadable `pm path` or `sha256sum`, `pm path` listing nothing, or a timeout), the install record is used as before and every summary or refusal says so: **…; the device could not hash its APK (…)**.
- **The crash's device.** Entries do not carry a serial. Each logcat stream start records the first entry ID it can store and its serial (`LogcatStateInner::record_stream_start`, at most `MAX_STREAM_STARTS`, 32); only the stream owning the current generation stores entries, so a crash belongs to the last start at or before its first entry (`device_of_entry`). A stream started without a serial read adb's only device: the one online device now is used, and several or none is refused. An entry older than every remembered start is refused. An emulator is identified by its AVD name (asked through `installed_builds::resolve_target`); one that does not answer is refused.
- **The mapping of an install record** (a hash match on one, or the fallback). `installed_build(target, package)` must exist and name a build (`buildId`); an install of an APK no recorded build wrote, or one without mappings (not minified, or the mapping was not rewritten), is refused. Mappings whose snapshot file is gone are ignored, and none left is refused. Several are narrowed to the installed APK's module and variant from the build's `BuiltApk` whose hash is the installed one; still several is refused with the list.
- **Asking the device (fallback).** `installed_builds::verify_install_on_device`, the one check debug sessions also use: `adb shell dumpsys package <package>` (the package validated and quoted with `quote_device_shell_arg`) and `adb shell date +%s:%z`, each with `ADB_QUERY_TIMEOUT`. The `Package [<package>]` block's `versionCode` must equal the recorded one (when recorded), and its `lastUpdateTime` (device local time) converted with the device's UTC offset and corrected for its clock's difference from the host's must not be later than the recorded `installedAt` plus `UPDATE_TIME_TOLERANCE_SECS` (30 s): otherwise "the app was reinstalled outside Keynobi after build #N". A package that is not installed is refused; a failed query or unreadable output is "could not check …", also refused. The check returns which of the three it was (`InstallMismatch`: `Unchecked`, `NotInstalled`, `Reinstalled`).
- **The tool.** Only under the SDK path from settings, never a project's `local.properties`: `cmdline-tools/latest/bin/retrace`, else the highest numbered `cmdline-tools/<version>/bin/retrace`, canonicalized and required to be an executable file. Its version is `Pkg.Revision` in that folder's `source.properties`. The JDK is the one Gradle builds use (`jdk::check_project_java`, so an untrusted project cannot choose it) and must be 17 or newer; it is passed as `JAVA_HOME` (not set when `java` on `PATH` was probed). A missing tool or JDK is `unavailable` with what to install, not an error.
- **Running it.** `retrace <mapping snapshot> <trace file>` through `output_with_timeout` with `RETRACE_TIMEOUT` (120 s), argument vector only. The trace goes through a `unique_tmp_path` file created `0600` with `create_new` in `<data dir>/retrace/`, removed by a guard whether the run succeeds, fails, or times out; files older than an hour there are removed first. A non-zero exit reports the first line of its stderr (else stdout) as `failed`; output over `MAX_RETRACE_OUTPUT_BYTES` (1 MiB) is `failed`. A trace over `MAX_RETRACE_INPUT_BYTES` (256 KiB) is refused before anything runs.
- **Cache.** Results are kept in memory per process by (mapping SHA-256, trace SHA-256), at most `MAX_RETRACE_CACHE` (32), least recently used dropped. Only the tool run is cached: the mapping is matched again every time, and the device asked again unless the trace has a map id.
- **MCP.** `retrace` defaults to false and leaves the outputs as they were. `get_crash_stack_trace` adds a `retrace` object (`status`, `mapping_line`, `trace`, `build_id`, `module`, `variant`, `map_id`, `mapping_sha256`, `matched_by` (`map_id`, `device_hash`, or `install_record`), `device`, `reason`); `get_crash_logs` adds `retraced`, one such object with its `crash_group_id` for each of the newest `MAX_RETRACED_CRASH_GROUPS` (5) crash groups among the entries it returns.
- **UI.** **Deobfuscate** appears in Entry Detail for an entry in a crash group. The result shows under the message: an `Alert` titled **Deobfuscated** (the summary, **Copy stack**, then the stack), **Not deobfuscated** or **Retrace not available** (the reason), or **Retrace failed** (a failed run or IPC error). Selecting another crash clears it and drops an answer still on its way.
- **Health.** `SystemHealthReport.retraceVersion` (the Command-line Tools version, `unknown`, or `null` when missing) is the **R8 retrace** row, a warning when missing; MCP `run_health_check` reports `checks.retrace`.

### Debug Sessions

`services/debug_sessions.rs` keeps a debug session per install epoch: one build installed on one device for one package, and what happened to it until the next install. Crashes and ANRs belong to installs, not builds, so this is what a crash will be attributed to.

- **Opening.** `installed_builds::install_and_record` opens a session after `record_install_in` succeeded, in the same blocking task, for both front doors (Run App and MCP `install_apk`, attached or standalone). The caller names who installed (`BuildActor`); `recordedBy` is `standalone` for a standalone MCP server's agent and `app` otherwise. The manifest references the build record by id and keeps a digest that outlives it (`DebugSessionBuild`: task, start time, origin, the installed APK's module, variant, SHA-256, and version code, and the install's mapping SHA-256s and map ids); it never copies the record. An APK no recorded build wrote opens a session with `build: null`. A failure to write a session is logged and never fails the install.
- **Closing.** A later install on the same device and package closes the open session as `superseded`. The device is matched as for installed builds (`InstallTarget::is_same_device`): by AVD name for emulators, by serial otherwise. **End session** (`end_debug_session`) closes it as `ended`. A session with no event for `SESSION_IDLE_SECS` (24 h) is `idle`: readers compute it (`closedAt` is the last event plus 24 h), and the next write under the lock records it. There is no timer.
- **Timeline.** `events.jsonl` holds one `DebugSessionEvent` per line: `seq`, `at` (host time), `actor`, `kind`, `data`. Opening writes `build` (when matched) and `install`. Hooks add:
  - `launch`: `record_launch` from the Tauri `launch_app_on_device` and MCP `launch_app`, `record_restart` from MCP `restart_app`; with the `LaunchTiming` when the device measured one (a restart's `display_time_ms` goes in its `displayedMs`). It resolves the device in a spawned task (an emulator is asked for its AVD name, as installs are), so the launch never waits for it.
  - `launchTiming`: display times that `record_late_display_times` recorded on the build after the launch returned (see § Launch Time), queued by `record_late_launch_timing` with the whole updated `LaunchTiming`. The log is append-only, so the `launch` event keeps the times known when it was recorded; a reader matches the two by `measuredAt`. It does not count as a launch.
  - `logcatReconnect`, `logcatStopped` (with the reason, at most `MAX_EVENT_TEXT_CHARS`), `logcatCleared`: from the `logcat:reconnecting` emit site, `give_up`, and `request_clear`, for the open sessions whose device is the stream's serial. The AVD name comes from the emulators this process last saw (installs, launches, and the device poll; `MAX_KNOWN_EMULATORS`, 64).
  - `deviceOffline` / `deviceOnline`: `DevicePresence` in the GUI device poll (`start_device_polling`) compares each polled list with the previous one; the first list is the baseline, and an emulator counts only once its AVD name is known.
  - `bookmark`: `add_session_bookmark(sessionId?, note, logEntryId?)`. Without a session id it takes the newest open session on the selected device, or on any device when none is selected. The note is trimmed, required, and at most `MAX_BOOKMARK_NOTE_CHARS` (500); a closed session refuses it.
  - `crash` / `anr`: from the logcat store (see Crash capture below), with `DebugSessionCrash`: serial, pid, `summary` (the exception line, or the `ANR in` line, at most `MAX_EVENT_TEXT_CHARS`), `signature` (a hash of the first line), `receivedAt` (host time), `deviceTime` (as logcat printed it), `attribution`, `capture`, and `droppedLines` (the stream's `LogStats.dropped_lines` when it was captured).
  - `exit`: a process exit Android recorded (`DebugSessionExit`: the `AppExitRecord`, `exitedAt` on the host clock, `matchedBy`); see Exit reasons below.
  - `agentAction`: an MCP tool call that acted on a device (`DebugSessionAgentAction`: `tool`, `kind` from its annotations (`write`, `destructive`, `openWorld`), `ok`, `durationMs`, `serial`; the agent is the actor, and arguments are never recorded). `LoggingMcpServer::call_tool` records it after the call returns, attached or standalone, through `record_agent_action`, for tools that are not read-only and take a device parameter (`device_serial`, `deviceSerial`, or `serial`): on the open sessions of the named device, only those of `package` when the call named one. A call that names no device uses the one online device this process knows (the app's device poll; a standalone server has none, so records nothing). A hidden tool or a call refused for the project is not recorded. At most `MAX_AGENT_EVENTS_PER_SESSION` (500) per session; later ones are dropped and counted.
- **Hooks never block their caller.** The logcat, device-poll, and agent-action hooks do in-memory work only and queue the event on a bounded channel, `MAX_PENDING_SESSION_EVENTS` (256), drained by one writer thread that does the file I/O under the data lock; a full queue drops the event and counts it (`dropped_session_events`, logged). A write failure is logged by the writer; the caller has already returned.
- **Crash capture.** In the logcat batch store loop (`store_and_filter_processed_entries`), the first stored entry of a crash group newer than the last one seen (`crash_groups_seen`; group ids only grow) becomes a `CrashSeen`: group id, entry id, pid, package, whether the pipeline flagged it `ANR`, host receive time, and `device_of_entry`. The pipeline task queues it after it releases the logcat lock (`CrashSource::record`, `try_send` on a channel of `MAX_PENDING_CRASHES`, 64; a full queue drops and counts, `dropped_crashes`). No I/O, no lock across an `.await`. One thread with its own runtime handles the queue in order: it waits until `CRASH_SETTLE` (2 s) after the crash arrived so the group can finish, then, holding the logcat lock only to copy, takes up to `CAPTURE_CONTEXT_BEFORE` (500) entries before the first line (`LogStore::context_before`) and the group's entries among the next `CAPTURE_SCAN_ENTRIES` (`LogStore::crash_group_from`), and for an ANR ActivityManager's `PID:` line after it. Outside the lock it keeps the group first, then the nearest earlier lines, within `MAX_CAPTURE_ENTRIES` (1,000) and `MAX_CAPTURE_BYTES` (1 MiB) (`truncated` says whether anything was cut), and writes them as JSON lines to a `unique_tmp_path` file in `sessions/` without the data lock (`PreparedCapture`, removed when dropped unpublished). Under the lock the file is renamed to `<id>/captures/crash-<seq>.jsonl`, named by the event's `seq` (group ids are per process), while the session has fewer than `MAX_CAPTURES_PER_SESSION` (10) captures (`counts.captures`); later crashes get the event without a capture. A crash whose first line left the buffer (cleared) or whose package is unknown is logged and not recorded.
- **Attribution.** The package is, for an ANR, the process `ANR in` names; otherwise the entry's package (the PID map), else AndroidRuntime's `Process:` line; a process name (`pkg:remote`) gives its package, which must pass `validate_package_name`. The device is `device_of_entry`'s serial (a stream without one uses the one online device, as retrace does; an unknown stream is not recorded), an emulator named by its AVD (asked now, else the one this process last saw on the serial). The session is the newest one with a Keynobi install on that device and package open at the host receive time (`logcat -T 1` is live, so host time is reliable). The device is then asked with `verify_install_on_device`: confirmed is `attribution: {method: installRecord, verified: true}`; a device that cannot be asked or no longer lists the package is `verified: false` with the reason, on the same session; a reinstall outside Keynobi, or no Keynobi install at all, goes to an **unattributed** session (`method: unattributed`, `build` and `install` null) of that device and package, created lazily (opened at the crash's receive time, same caps and retention) and reused while it is open. A session superseded between the crash and the check is not asked (`verified: false`).
- **Dedupe.** Two processes streaming one device both see a crash, and a reconnect can replay a line. Under the data lock, before recording, the last `DEDUPE_TAIL_BYTES` (64 KiB) of each session of the device and package active in the last 5 s are searched: the same pid and `signature` with a `receivedAt` within `CRASH_DEDUPE_SECS` (5 s) is the same crash, and is dropped (with its capture).
- **Exit reasons.** After a crash or ANR is recorded, its session's exit reasons are read once, `EXIT_READ_DELAY` (5 s) later, through `app_exit_info::read_exit_reasons` (API 30+; older devices are skipped). Reads are debounced per session: while one waits, later crashes do not schedule another (`MAX_PENDING_EXIT_READS`, 16 sessions). `refresh_session_exit_reasons(id)` reads them on demand from the session's device (an emulator must still run the session's AVD on its serial). Each record's `timestampLocal` is converted with the device's UTC offset and clock skew (`installed_builds::device_clock`, `DeviceClock::to_host_time`). A record belongs to the session when that time falls within [install time, `closedAt`) (a session without an install starts at its first crash less `UNATTRIBUTED_EXIT_TOLERANCE_SECS`, 2 s); it is `matchedBy: pid` when its pid is one of the session's crash or ANR events, `processName` when its process is the package, else `timeWindow`. Records already recorded (same timestamp, pid, and reason) are skipped.
- **Storage.** `<data dir>/sessions/index.json` (`schemaVersion` 1, a `DebugSessionSummary` per session; the only file the list reads), `<id>/session.json` (the `DebugSession` manifest, `schemaVersion` 1), `<id>/events.jsonl`, and `<id>/captures/crash-<seq>.jsonl` (the log lines kept with crash event `seq`, one `ProcessedEntry` per line). Ids are `s-<UTC second>-<48 random bits from /dev/urandom>`, and the folder is created with `create_dir`, so two processes never share one. Ids are validated (`validate_debug_session_id`) before they name a path. A missing or unreadable index is rebuilt from the manifests. Times are fixed-width RFC 3339 UTC, so they sort as text.
- **Cross-process.** Every read-modify-write of the index and a manifest, and every append, runs under `settings_manager::with_data_lock`, re-reading inside it and writing with `unique_tmp_path` and rename. The app and standalone MCP servers write the same files; the app sees a standalone server's sessions the next time it reads the index. An event is appended before the manifest and index are saved, so a reader can briefly see a line the index does not count yet.
- **Caps.** Per session: `MAX_EVENTS_PER_SESSION` (2,000), `MAX_BOOKMARKS_PER_SESSION` (100), `MAX_AGENT_EVENTS_PER_SESSION` (500), and `MAX_SESSION_BYTES` (16 MiB of events); an event past a cap is counted in `droppedEvents` (a bookmark past it is refused). `MAX_CAPTURES_PER_SESSION` (10) crash captures of at most `MAX_CAPTURE_ENTRIES` (1,000) lines and `MAX_CAPTURE_BYTES` (1 MiB) each, outside `bytes` and `MAX_SESSION_BYTES`. `get_debug_session` returns the newest `MAX_EVENTS_RETURNED` (500) with `eventsTruncated`, and the newest `MAX_CRASHES_RETURNED` (100) crash and ANR events in `crashes` even when they are older; `list_debug_sessions` returns at most `MAX_SESSIONS`, newest first.
- **Retention.** `prune_locked`, in order: at most `MAX_KEPT_SESSIONS` (5) stay kept (the oldest past it are unkept); unkept sessions whose last event is older than `sessions.retentionDays` (default 14; 0 disables) are removed; then, oldest first (closed before open), sessions until the folder is under `sessions.maxFolderMb` (default 200); then the oldest past `MAX_SESSIONS` (50). Kept sessions and the session being opened are never removed. Folders the index does not name and temporary files older than an hour are removed. Pruning runs at startup (next to `rotate_persisted_build_logs`), when a session opens (and supersedes one), and when one is ended. Both settings are clamped on load (`normalize_sessions_section`: at most 365 days, 10–2,048 MB).
- **Mappings.** Sessions reference mappings by SHA-256 and do not pin them, except a session marked Keep (`set_debug_session_kept`), which adds its mapping SHA-256s to `KeptMappings.pinned` through `mapping_snapshots::mappings_to_keep`, the one retention rule.
- **Commands.** `list_debug_sessions`, `get_debug_session(id)`, `end_debug_session(id)`, `set_debug_session_kept(id, kept)`, `add_session_bookmark`, `get_session_capture(id, seq, limit?)` (the newest `limit` lines of a capture, at most `MAX_CAPTURE_ENTRIES`, which end with the crash; `NotFound` when none was kept), and `refresh_session_exit_reasons(id)` (`DebugSessionExitRefresh`: how many exits were added, or why none could be read) (`commands/sessions.rs`) are thin wrappers that run the service on a blocking task and return `AppError` (`InvalidInput` for a bad id, note, or a sixth kept session; `NotFound` for a pruned session).
- **MCP tools.** `list_debug_sessions`, `get_debug_session`, and `compare_debug_sessions` (core toolset, read-only) call the same service as the commands, through `debug_sessions/agent.rs` and `debug_sessions/compare.rs`, on a blocking task. The list filters by package, device (serial or AVD name), state (`open`, `closed`, `superseded`, `ended`, `idle`), and `only_crashing`, and returns one line per session (`agent_line`), at most `MAX_AGENT_SESSIONS` (default `DEFAULT_AGENT_SESSIONS`, 10), with how many matched. `get_debug_session` returns the summary and build digest, a page of the timeline (`DEFAULT_AGENT_EVENTS`, 100, at most `MAX_AGENT_EVENTS`, 500; one line per event, newest page first, oldest first within it, `next_before_seq` as the cursor for older ones), the newest `MAX_AGENT_CRASHES` (20) crashes and ANRs with their attribution (`install_record` or `unattributed`, `verified`, reason) and `crashes_truncated`, and with `capture_seq` that crash's newest `log_lines` kept lines (default 100). `compare_debug_sessions(from, to)` (`compare_sessions`) compares the build digests (build id, task, module, variant, version code, APK and mapping hashes), device, and installer; the latest launch of each (a time difference only on the same device and launch state); crash and ANR signatures (at most `MAX_COMPARED_SIGNATURES`, 20, marking those new in `to`); and exit reasons by count. Without ids it picks the default pair (`default_pair`): the newest session with a launch and no crash or ANR that has a later crashing session of the same project, package, module, and variant, and the first such session. `provenance` is always `null` and `not_recorded` lists what a build does not record yet (source commit, build-file hashes, dependency versions, AGP version, uncommitted changes). A bad id or half a pair is `invalid_params`; a pruned session or no default pair is a tool error.
- **UI.** **Show Debug Sessions** (Command Palette) opens `components/sessions/SessionsDialog.tsx`, and the past-build bar links to the newest session of the viewed build's APK on each device (**Session: 2 crashes on Pixel_7**; `sessionsOfBuild` requires the session's `apkSha256` among the record's `apks`, as installs do). The dialog is a modal (`modalFocus`): a `Listbox` of `list_debug_sessions` (build, device, package, state, counts, and **Kept**, **Standalone**, **Unattributed** badges) and the selected session's detail. There is no session event: while the dialog is open it reads the list again every `SESSIONS_POLL_MS` (3 s), so a standalone server's sessions appear, and reads the selected session again only when its summary changed (events, closed, kept). The timeline is a `VirtualList` of fixed-height rows (a session has up to `MAX_EVENTS_PER_SESSION` events; a read returns the newest `MAX_EVENTS_RETURNED` plus every crash) and one tab stop (`role="listbox"` with `aria-activedescendant`; Up/Down, Home, End select; Enter opens a crash's lines); an `agentAction` is an **Agent** row with the tool, the agent, the duration, and whether it failed. The selected event shows in full below it; a crash shows its attribution and its kept lines through `get_session_capture`, the newest `CAPTURE_PAGE_LINES` (200) first and 200 older per **Load older lines**, up to the capture's size. Actions: **Keep** (the sixth is refused inline with the backend's message), **End session**, **Refresh exit reasons** (says how many exits were added, or why none could be read), and **Add bookmark** (at most `MAX_BOOKMARK_NOTE_CHARS`, checked before the call). Buttons stay enabled while an action runs (a second waits), and focus that a disabled control dropped returns to the first action.
...
- **Run App** and **Build Only** build the active run configuration's module only (see [Run Configurations](#run-configurations)): its task, by default `:<module>:assemble<Variant>`, or `assemble<Variant>` for the root project. An unqualified `assemble<Variant>` would run in every module.
...
A run configuration says what Run builds and launches: `RunConfiguration` (`models/run_configuration.rs`: `name`, `module`, `variant`, `task`, `launch`, `logcatFilter`). It is portable, with no paths, serials, or environment. What is specific to this machine is kept apart in `LocalRunState`: the `target` (`TargetPreference`: `ask`, `serial`, `avd`, `lastUsed`), the last device, and the approved hash of a shared file.

- **Storage.** Locally, in the project's registry entry: `ProjectEntry.runConfigurations`, `runLocal` (by name), and `activeRunConfiguration`, all `#[serde(default)]` and left out of the file when unset. `services/run_configurations.rs` writes them only through `mutate_settings`, re-reading the entry under the data lock, so saves from the app and a standalone MCP server are both kept. `save_settings` (the settings UI's snapshot) keeps `recentProjects` from disk, so it can never write them.
- **Migration.** An entry without `runConfigurations` (saved by an older version, or never read) gets them on its first read (`list_run_configurations`, or before a save, delete, or activation). The project's files are read before the data lock (`migration_seed`: the application modules, at most `MAX_RUN_CONFIGURATIONS`, and the variants each build file declares); `migrate` then runs under the lock and only when the entry still has none. One application module gives a configuration named **Default**, active; several give one per module, named after the module (its whole path when two share a last segment), and none active until one is chosen. The variant is `lastBuildVariant` when the module declares it (or declares none), else the module's default variant, else `debug`. Launch is `default`; the target is `lastUsed` with `lastDevice`. A project with no application module is left unmigrated. Deleting every configuration keeps an empty list, which is never migrated again.
- **Old fields.** `lastBuildVariant` and `lastDevice` stay. Saving or activating a configuration sets `lastBuildVariant` to the active configuration's variant, and the variant picker (Tauri and MCP `set_active_variant`) sets the active configuration's variant (`follow_variant`), so an older Keynobi and MCP readers see the same variant.
- **Validation.** `validate_run_configuration` (reused by every future front door): the name is not empty, has no surrounding spaces or control characters, is at most `MAX_RUN_CONFIGURATION_NAME_CHARS` (64), and is unique ignoring case (a save with the same name replaces that configuration); the module is one of `gradle_modules::application_modules`; the variant is letters and digits starting with a letter; the task passes `validate_gradle_task` and names the module (`:mobile:bundleRelease`; no module path for the root project); an activity passes `validate_activity_name` and a deep link `validate_deep_link_uri`; the logcat filter is one line of at most `MAX_LOGCAT_FILTER_BYTES` (1 KiB). A project has at most `MAX_RUN_CONFIGURATIONS` (32). A refused save writes nothing.
- **Commands.** `list_run_configurations(projectRoot?)` (default: the open project), `save_run_configuration(config)`, `delete_run_configuration(name)`, `set_active_run_configuration(name)`, and `record_run_device(name, serial)` (sets `lastDevice`) return the project's `ProjectRunConfigurations` (configurations, active name, local state). A project not in the registry is `notFound`; a refused configuration `invalidInput`. Deleting the active configuration leaves none active.
- **Resolution.** Before anything runs, `resolve_run_configuration(name?, selectedSerial?, buildOnly?)` (`services/run_plan.rs`, one service for every front door) turns the named configuration, else the active one, into a `ResolvedRun`: module, variant, task, launch, logcat filter, the device (`RunDevice`: serial and label, the AVD name, else the model, else the serial; none for `buildOnly`), and the plan in one line, for example `Run 'Default': build :app:assembleDebug → install this build's APK → launch the app on Pixel_7 → filter package:mine`. It checks, in order:
  - a configuration is named or active; none active is `invalidInput` listing the configurations (`mobile (:mobile debug), wear (:wear debug)`), which is how several application modules stop until one is chosen; an unknown name is `notFound`;
  - the configuration against the project as it is now, with `validate_run_configuration`: the module is an application module, the task is valid and in the module, the activity or deep link is valid; and the variant is one the module's build file declares (not checked when it declares none, as in migration). Each is `invalidInput`, `Run configuration '<name>' cannot run: …`;
  - the project is trusted (`require_trusted`; `permissionDenied` in Safe Mode);
  - the target, to exactly one online device, from the polled device list: `serial` only when that device is online; `avd` the one online emulator whose `avdName` equals it exactly (none: **… runs on the AVD Pixel_7, which is not running. Launch it, then run again.**; the GUI offers to launch it, nothing starts an emulator on its own; several emulators of the AVD are refused); `lastUsed` its `lastDevice` when online, else the selected device; `ask` the selected device. The selected device is `selectedSerial` (the app's selection, or the device just picked for this run), else the backend's. No device is `notFound`.
- **Run App.** `runAndDeploy` resolves the active configuration first. For a target of `ask` or `lastUsed` that finds no device, it shows the device picker and resolves again with the picked device; other failures stop Run App with the reason in the build log. It builds the plan's task, installs the APK with the rule in [Application Module](#application-module), records the device (`record_run_device`, through `mutate_settings`), and launches per `launch`: `default` the launcher activity, `activity` that activity (`launch_app_on_device`), `deepLink` `open_deep_link_on_device` (`am start -a VIEW -d <uri> -p <package>`, which reports no launch time), `none` nothing. After a launch, `setLastLaunchedAt(package, logcatFilter)`: the Logcat panel replaces its query with the configuration's filter, or merges `package:mine` into it when there is none (`queryAfterLaunch`). **Build Only** resolves with `buildOnly` (no device) and builds the plan's task.
...
- **Crash groups.** `FATAL EXCEPTION`, `Uncaught handler`, native signals, and ANRs open a group; following lines stay in it while they are stack lines (leading tab or `  at `), look like exception output (`Exception`, `Error`, `Caused by`, `...`), or are AndroidRuntime's `Process: <package>, PID: <pid>` line. Any other line closes it.
...
- Each start that spawns a stream records its serial and the next entry ID (`record_stream_start`, at most `MAX_STREAM_STARTS`), so the device a stored entry came from is known (`device_of_entry`); see [Retrace](#retrace).
...
- Across processes, the call then also takes `<data dir>/ui-automator-locks/<hash of the serial>.lock`, an advisory file lock like the build lock (`build_lock::try_lock_file`), and holds it until the capture ends. It is retried every `CROSS_PROCESS_POLL` (50 ms) until the capture's deadline; a device another process still holds then fails with "busy with a request from another Keynobi process (pid N)". The lock dies with its process, so a crashed holder never blocks a device. It is keyed by serial, the name adb addresses the device by, not by AVD name: resolving the AVD name would cost an adb call per capture, and at any moment a serial names one device. A lock file that cannot be created (an unwritable data directory) is logged and the capture runs without it.
...
- **Mapping snapshots.** An incremental build whose minify task was up to date does not rewrite `mapping.txt`, so it records no mapping, although its APK matches the one an earlier build saved. Freshness is an mtime comparison with the build start, which assumes the filesystem keeps sub-second mtimes (APFS does). The `MAX_MAPPING_SNAPSHOTS` backstop can remove a snapshot a record still names (never one an installed build names); the record keeps listing it. The folder has no total-size setting (32 × 256 MiB at worst). Mappings are found only at `mapping/<variant>/mapping.txt`, one directory deep. Pinned snapshots add up to `MAX_INSTALLED_TARGETS` × 256 MiB to that worst case.
- **Installed builds.** Only installs through Keynobi (Run App, MCP `install_apk`) are recorded; an APK installed by Android Studio or `adb install` by hand leaves a stale entry (retrace checks the device's APK hash against it, and recognises such an APK when a kept build wrote it). Split APKs and app bundles are not handled: every APK of a build is hashed, but an install is one APK and an app bundle is not an APK. An incremental build that did not rewrite the APK records no APK, so its install matches the earlier build that wrote it (the right mapping), or none once that build left the history. The emulator's AVD name is asked after the install, so an emulator replaced on the same serial during an install could be misnamed. An install whose package cannot be read (no output metadata and no aapt2) is not recorded. A corrupt `installed-builds.json` reads as empty, releasing its pins.
- **Retrace.** Split APKs and app bundles are refused without a map id in the trace: only a single installed APK is hashed, and a bundle's APKs are not built as APKs Keynobi hashes. An APK no kept build wrote and Keynobi never installed (a CI build, a build that left the history) has no known mapping unless its frames carry a map id. The version of AGP that first ships the R8 default of `r8-map-id-<pg_map_id>` source files was not verified; older builds (and builds keeping `SourceFile` or setting `-renamesourcefileattribute`) have no map id and go to the device hash. A custom map id (R8's map-id template) with characters other than letters and digits, or over 64 characters, is not saved (`parse_pg_map_id`), so a trace naming it is refused. `sha256sum` reads the whole APK on the device (bounded by `ADB_APK_HASH_TIMEOUT`, then the fallback). On the fallback: an app reinstalled outside Keynobi with the same version code within 30 s of Keynobi's install is not caught, and a device whose UTC offset changed since the install (daylight saving) shifts `lastUpdateTime` by the difference, which can refuse a good install. Time on large mappings is not measured (two JVM starts, about 0.6 s with a small mapping; 50–150 MB mappings are parsed on every uncached call, bounded by `RETRACE_TIMEOUT`). Crash groups end at the first line that does not look like exception output, so an exception class without `Exception` or `Error` in its name is cut off before its frames. The cache is per process and lost on restart. A stream started without a serial is matched to the one device online now, which may not be the one that was streamed.
- **Debug sessions.** Bookmarks store their `logEntryId` without capturing the log around it. The Sessions dialog polls rather than listening for a change event, has no Logcat toolbar or palette **Bookmark Session** action (its bookmarks name no log entry), does not deobfuscate a captured crash (a capture's crash group ids belong to the process that captured it, so the retrace path, which reads the live buffer, cannot take them), and Entry Detail does not yet say which build a crash came from. Installs outside Keynobi open no session and leave the last one open; a crash that shows the reinstall goes to an unattributed session beside it.
- **Debug session MCP tools.** `list_debug_sessions` has no project filter and no launch time or verified state per line (they would need each session's events); `get_debug_session` does not deobfuscate a crash. `compare_debug_sessions` compares only what a session records: build provenance (commit, build files, dependencies) is not recorded, so `provenance` is `null`. Agent actions: a tool that names no device records only when this process knows exactly one online device, so a standalone server records none for such calls; an emulator's session is found through the AVD name this process last saw on the serial; `launch_app` resolves its launch in a spawned task, so its `agentAction` can come before its `launch`; tools that act on a device without a device parameter (`launch_avd`, `run_tests`) are not recorded.
- **Debug session crashes.** Only a process that streams logcat records crashes. A crash is not recorded when its lines left the buffer before `CRASH_SETTLE` ended (a clear), when logcat named no package for it, or when its stream is older than every remembered stream start; dropped queue entries are counted per process and only logged. An emulator that does not report its AVD name falls back to the name this process last saw on the serial, else its serial, which matches no AVD-named session, so its crash goes to an unattributed session. The pipeline gives a crash group the line that ends it, so a capture's last line can be unrelated. An ANR's pid comes from ActivityManager's `PID:` line within 10 entries, else it is `null`. Captures are not counted in a session's `bytes`. Duplicates are found only in the last 64 KiB of the event logs. The writer handles crashes one at a time, so a device that does not answer delays the next capture by up to `ADB_QUERY_TIMEOUT` per query, and a ring buffer that wraps meanwhile loses its lines.
- **Debug session exits.** Exit reasons are read only after a crash this process recorded, or on `refresh_session_exit_reasons`; exits without a crash (a low-memory kill) wait for a refresh, and a read adds records only to the session that asked. Launch events carry no pid, so only crash and ANR pids match `pid`; `processName` matches every exit of the main process, so the time window is what separates sessions. A device whose UTC offset changed since the exit (daylight saving) shifts its time by the difference, and records without a readable `timestampLocal` are not attributed. Logcat events of a stream started without a serial are not attributed, and an emulator's AVD name for them is the one this process last saw on the serial (a different AVD on a reused serial, seen by neither an install, a launch, nor the device poll, is taken for the old one). `deviceOffline`/`deviceOnline` come from the GUI device poll only; standalone servers do not poll. A session whose build left the history between recording the install and opening the session gets no build digest. Dropped queue events are counted per process and only logged. Every event rewrites the manifest and the index.
- **Past builds lack variant and device.** `BuildRecord` does not store the variant (only the APKs it wrote name theirs), and stores the target device only inside a launch time (installs are kept per device, not on the record), so a past build is described by its task (which names the variant), and a past cancelled build shows no duration. The live build's install step, and a launch that reported no time, are not recorded.
- **Launch time scope.** The display times need this process's logcat stream on the launched device; without it they are not recorded, and Keynobi does not start a stream for them. A `Displayed` or `Fully drawn` line the device logged just before the launch but that reached the stream after it started would count, since entries are ordered by arrival, not by device time. MCP `launch_app` reports only `am start -W`'s times. A standalone MCP server never records launch times, and a launch time recorded after another process rewrote the history file is kept only if the record is still among the last `MAX_HISTORY` builds.
...
- **UI Automator across processes.** Captures take turns across processes through the device's lock file, but the instrumentation check is per process: a connected test run started by one process is invisible to another, whose capture then gets the device's "already registered" error, reported as busy. The lock is keyed by serial, so one device reached through two serials (USB and wireless ADB) is not serialized. When the lock file cannot be created, the capture runs without it.
...
- **Several application modules.** Nothing in the app chooses the active run configuration yet, so Run App and Build Only stop with the list of configurations; the variant preview and App Info fail with the list of modules, and variant discovery with Gradle, asked for no module, lists every module's tasks. Variant discovery, `get_application_module`, and `find_apk_path` take a module, but nothing in the UI sets `variantState.module` yet. MCP `find_apk_path` takes a `module`; `get_build_config` takes a directory name, not a nested module path.
- **Run configurations have no UI or MCP tools.** Nothing in the app shows, edits, or chooses them, and no MCP tool reads them; Run App and Build Only use the active one. Only migration sets a target (`lastUsed`), and `approvedProjectFileSha256` is not updated yet. When the variant picker changes the active configuration's variant, a custom `task` is left as it was. An older Keynobi that saves the registry drops the new fields; the next read migrates again from `lastBuildVariant`. A deep-link launch adds no launch event to the debug session.
- **Run resolution limits.** A variant is checked only against the variants the module's build file declares, so a variant a convention plugin adds passes unchecked when the file declares none, and is refused when the file declares others. Build, install, and launch are orchestrated in the frontend (`runAndDeploy`), so no MCP tool can run a configuration. The device list is the poller's, so a device that came online in the last few seconds can be reported as not online.
...
- **Exit reasons are not linked to stacks.** An exit record is matched to a debug session (see [Debug Sessions](#debug-sessions)), not to the logcat crash group that caused it, and ANR traces (`/data/anr`) and tombstones are not read. Times are the device's local time with no offset. Records of every user (work profile) are listed together, without the user.
