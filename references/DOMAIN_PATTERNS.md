...
- **Reading.** `installed_build(target, package)` returns the entry for a device and package (for the deobfuscation work to come). The frontend reads the whole list with `list_installed_builds`.
