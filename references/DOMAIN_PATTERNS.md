...
- **Hooks never block their caller.** The logcat and device-poll hooks do in-memory work only and queue the event on a bounded channel, `MAX_PENDING_SESSION_EVENTS` (256), drained by one writer thread that does the file I/O under the data lock; a full queue drops the event and counts it (`dropped_session_events`, logged). A write failure is logged by the writer; the caller has already returned.
...
- **Commands.** `list_run_configurations(projectRoot?)` (default: the open project), `save_run_configuration(config)`, `delete_run_configuration(name)`, `set_active_run_configuration(name)`, `set_run_configuration_target(name, target)` (a `serial` passes `validate_device_serial`, an `avd` `validate_avd_name`), and `record_run_device(name, serial)` (sets `lastDevice`) return the project's `ProjectRunConfigurations` (configurations, active name, local state). A project not in the registry is `notFound`; a refused configuration `invalidInput`. Deleting the active configuration leaves none active. `list_application_modules()` returns the open project's application module paths, for the editor's module picker.
...
- **Run App.** `runAndDeploy(name?)` resolves the named configuration (the palette's **Run: <name>**), else the active one, first. For a target of `ask` or `lastUsed` that finds no device, it shows the device picker and resolves again with the picked device; for an `avd` target that is not running, it offers **Launch AVD** (`launchRunAvd`, the existing `launch_avd`) and stops, so the user runs again once it is online; other failures stop Run App with the reason in the build log. It builds the plan's task, installs the APK with the rule in [Application Module](#application-module), records the device (`record_run_device`, through `mutate_settings`), and launches per `launch`: `default` the launcher activity, `activity` that activity (`launch_app_on_device`), `deepLink` `open_deep_link_on_device` (`am start -a VIEW -d <uri> -p <package>`, which reports no launch time), `none` nothing. After a launch, `setLastLaunchedAt(package, logcatFilter)`: the Logcat panel replaces its query with the configuration's filter, or merges `package:mine` into it when there is none (`queryAfterLaunch`). **Build Only** (`runBuildOnly(name?)`) resolves with `buildOnly` (no device) and builds the plan's task.
- **Frontend.** `stores/run-configurations.store.ts` holds the open project's `ProjectRunConfigurations`; `services/run-configurations.service.ts` is its only writer. A project open (`reloadVariantsAndRestoreMeta`) loads them and makes the variant picker follow the active configuration (its module when the project has several application modules, and its variant). The title bar picker (`RunConfigurationPicker`, a `Select`) chooses the active one; `RunConfigurationsDialog` edits them, validating the draft first (`run-configuration-draft.ts`: a name, a task, the activity or deep link, and the logcat filter with `validateLogcatQuery`) and showing the backend's refusal inline. Its **Resolved plan** calls `resolve_run_configuration` for the saved configuration. A save is `save_run_configuration` then `set_run_configuration_target`; a rename saves the new name, then deletes the old one and re-activates the new one when the old was active. Every configuration gets **Run: <name>** and **Build: <name>** palette actions, replaced on each load, so a project switch removes the previous project's. The build service registers itself as their runner (`setRunConfigurationRunner`) to avoid an import cycle.
...
- **Debug sessions.** Bookmarks store their `logEntryId` without capturing the log around it. There is no MCP tool to read sessions. The Sessions dialog polls rather than listening for a change event, has no Logcat toolbar or palette **Bookmark Session** action (its bookmarks name no log entry), does not deobfuscate a captured crash (a capture's crash group ids belong to the process that captured it, so the retrace path, which reads the live buffer, cannot take them), and Entry Detail does not yet say which build a crash came from. Installs outside Keynobi open no session and leave the last one open; a crash that shows the reinstall goes to an unattributed session beside it.
...
- **Several application modules.** Until a run configuration is chosen in the title bar, Run App and Build Only stop with the list of configurations and the variant picker lists no module's variants; App Info fails with the list of modules. Only choosing a configuration sets `variantState.module`. MCP `find_apk_path` takes a `module`; `get_build_config` takes a directory name, not a nested module path.
- **Run configurations have no MCP tools.** No MCP tool reads or changes them, and `approvedProjectFileSha256` is not updated yet. A rename is three writes (save, set target, delete the old name), so a failure between them leaves both names. The editor's variant list for a module other than the variant picker's comes from the build file preview, so it can miss variants a convention plugin adds. The editor's plan is the saved configuration's, not the unsaved draft's. When the variant picker changes the active configuration's variant, a custom `task` is left as it was. An older Keynobi that saves the registry drops the new fields; the next read migrates again from `lastBuildVariant`. A deep-link launch adds no launch event to the debug session.
