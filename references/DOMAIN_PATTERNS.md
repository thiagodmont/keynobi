...
- **Reading.** `installed_build(target, package)` returns the entry for a device and package; [Retrace](#retrace) chooses its mapping from it. The frontend reads the whole list with `list_installed_builds`.
...
`services/retrace.rs` deobfuscates a crash's stack trace with the Android SDK's R8 `retrace`, using the mapping of the build Keynobi installed on the device the crash came from. The Tauri command `retrace_crash(crashGroupId)` (**Deobfuscate** in Entry Detail) and MCP `get_crash_stack_trace`/`get_crash_logs` with `retrace: true` all call `retrace_crash_group`, which returns a `RetraceOutcome`: `status` (`retraced`, `unavailable`, `refused`, `failed`), the trace (deobfuscated, else as logcat printed it), the build, the `MappingSnapshot` used, `matchedBy` (`installRecord`), the device, the reason, and a one-line `summary` naming the build, module, variant, map id, and how the mapping was matched. Only a crash that left the logcat buffer is an error (`NotFound`); everything else is an outcome, so an agent's tool call never fails for it.

- **Never guess.** A wrong mapping gives plausible but wrong stacks, so every uncertainty refuses and returns the trace unchanged with the reason. Checked in this order: the trace size; the crash's package (from the logcat pipeline, validated); its device; the install record; the mapping; the device's answer. Only then is the cache consulted and the tool run.
...
- **Cache.** Results are kept in memory per process by (mapping SHA-256, trace SHA-256), at most `MAX_RETRACE_CACHE` (32), least recently used dropped. Only the tool run is cached: the device is asked again every time.
- **MCP.** `retrace` defaults to false and leaves the outputs as they were. `get_crash_stack_trace` adds a `retrace` object (`status`, `mapping_line`, `trace`, `build_id`, `module`, `variant`, `map_id`, `mapping_sha256`, `matched_by`, `device`, `reason`); `get_crash_logs` adds `retraced`, one such object with its `crash_group_id` for each of the newest `MAX_RETRACED_CRASH_GROUPS` (5) crash groups among the entries it returns.
