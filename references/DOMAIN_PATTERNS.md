...
- **Debug sessions.** Bookmarks store their `logEntryId` without capturing the log around it. There is no UI or MCP tool to read sessions. Installs outside Keynobi open no session and leave the last one open; a crash that shows the reinstall goes to an unattributed session beside it.
