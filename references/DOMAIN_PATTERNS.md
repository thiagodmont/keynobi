...
- **Retention.** After the history is saved, `prune_snapshots` removes every snapshot `mappings_to_keep` does not return: the hashes the kept history (re-read from disk, so other processes' records count, and capped at `MAX_HISTORY`) names (`KeptMappings.referenced`), and the hashes the saved installs name (`KeptMappings.pinned`, read from `installed-builds.json` in the same critical section; see [Installed Builds](#installed-builds)). Anything else that must outlive its record is added to that one function. It also runs on **Clear build history** (which keeps pinned snapshots) and at startup (`rotate_persisted_build_logs`), removes temporary copies older than an hour (a process that died mid-copy), and, past `MAX_MAPPING_SNAPSHOTS` (32) unpinned snapshots, removes the least recently saved unpinned ones (a dedupe hit counts as saved) with a warning. A pinned snapshot is never removed and does not count toward the backstop; there are at most `MAX_INSTALLED_TARGETS` (16) of them, one per install.
