...
- **Commands.** `list_run_configurations(projectRoot?)` (default: the open project), `save_run_configuration(config)`, `delete_run_configuration(name)`, and `set_active_run_configuration(name)` return the project's `ProjectRunConfigurations` (configurations, active name, local state). A project not in the registry is `notFound`; a refused configuration `invalidInput`. Deleting the active configuration leaves none active.
...
- **Run configurations are stored but not used.** Nothing in the app shows or edits them, and no MCP tool reads them: Run App still builds the variant picker's variant with the selected device, and ignores the task, launch, logcat filter, and target. Only migration sets a target (`lastUsed`); `lastDevice` in `LocalRunState` and `approvedProjectFileSha256` are not updated yet. When the variant picker changes the active configuration's variant, a custom `task` is left as it was. An older Keynobi that saves the registry drops the new fields; the next read migrates again from `lastBuildVariant`.
