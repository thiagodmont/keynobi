...
| `restart_app` | D | `package`, `device_serial?`, `clear_data?`, `allow_foreign_package?`. [Package-scoped](#package-scope). Force-stops and relaunches; app data is preserved. `clear_data: true` runs `pm clear` first (wipes data and runtime permissions) and requires `device_serial`. The removed `cold` parameter returns an error. Returns `total_time_ms`, `wait_time_ms`, and `launch_state` from `am start -W` (`null` when the device reports none) and `display_time_ms` from the logcat `Displayed` line (`ActivityTaskManager` or `ActivityManager`, the restarted package exactly; see `DOMAIN_PATTERNS.md` § Launch Time). Does not write build history. |
| `list_avds` | R | |
