...
// ── Compiled regexes ──────────────────────────────────────────────────────────
//
// Hoisted to LazyLock statics (matching build_parser.rs) so parsing a Gradle
// file does not recompile them on every call.

static FLAVOR_DIMENSIONS_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(|| {
    regex::Regex::new(r#"flavorDimensions\s*\(([^)]+)\)"#).expect("FLAVOR_DIMENSIONS_RE")
});

static FLAVOR_SUBBLOCK_RE: std::sync::LazyLock<regex::Regex> = std::sync::LazyLock::new(|| {
    regex::Regex::new(r"(\w+)\s*\{([^}]*)\}").expect("FLAVOR_SUBBLOCK_RE")
});

static FLAVOR_DIMENSION_INNER_RE: std::sync::LazyLock<regex::Regex> =
    std::sync::LazyLock::new(|| {
        regex::Regex::new(r#"dimension\s*["'](\w+)["']"#).expect("FLAVOR_DIMENSION_INNER_RE")
    });

static ASSEMBLE_INSTALL_TASK_RE: std::sync::LazyLock<regex::Regex> =
    std::sync::LazyLock::new(|| {
        regex::Regex::new(r"^(assemble|install)([A-Z]\w+)").expect("ASSEMBLE_INSTALL_TASK_RE")
    });
...
fn extract_flavors(content: &str) -> (Vec<String>, std::collections::HashMap<String, Vec<String>>) {
    ...
    let dim_re = &*FLAVOR_DIMENSIONS_RE;
    ...
    if let Some(start) = prod_flavors_start {
        ...
        let sub_re = &*FLAVOR_SUBBLOCK_RE;
        let dim_inner_re = &*FLAVOR_DIMENSION_INNER_RE;
...
pub fn parse_variants_from_tasks_output(tasks_output: &str) -> VariantList {
    let re = &*ASSEMBLE_INSTALL_TASK_RE;
