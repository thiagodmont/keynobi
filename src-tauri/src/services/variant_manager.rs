...
pub fn parse_variants_from_gradle(
    ...
    // If nothing found, return minimal debug/release defaults.
...
/// Parse the output of `./gradlew :app:tasks --all --console=plain` and
/// extract assemble/install tasks as a fallback variant list.
pub fn parse_variants_from_tasks_output(tasks_output: &str) -> VariantList {
    ...
    let mut variants: Vec<BuildVariant> = assemble
        ...
        .map(|(suffix, assemble_task)| {
            // Infer build type as the last capitalized segment (naive heuristic).
            // e.g. "FreeDebug" → build_type = "debug"
            let name = {
    ...
    // Deduplicate by name.
    variants.dedup_by(|a, b| a.name == b.name);
