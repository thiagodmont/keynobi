...
const AGP_ERROR_PATTERN: &str = r"^ERROR: (.+\.(xml|java|kt)):(\d+): (.+)$";

// Gradle build exception header: `FAILURE: Build failed with an exception.`
const GRADLE_FAILURE_PATTERN: &str = r"^(FAILURE: .+|> Could not resolve .+|> Could not find .+|> Failed to resolve .+|> Configuration cache .+|Error while executing process .+|Caused by: .+)$";
...
const GENERIC_ERROR_PATTERN: &str = r"^(?:error|Error): (.+)$";
